...
class Pokemon
  ...
  def initialize(specie, name, level)
    ...
    @indivudal_stats = { hp: rand(0..31), attack: rand(0..31), defense: rand(0..31), special_attack: rand(0..31),
  ...
  def attack(target)
    ...
    if hits
      base_damage = ((((2 * level / 5.0) + 2).floor * offensive_stat(current_move[:type],
                                                                     target.type) * @current_move[:power] / target_defensive_stat(
                                                                       current_move[:type], target
                                                                     )).floor / 50.0).floor + 2
      # -- Critical Hit check
      ...
      multi = offensive_stat(current_move[:type], target.type)
      # -- Effectiveness check
      if multi == 0.5
        current_damage *= 0.5
  ...
  def increase_stats(target)
    ...
    if @current_exp >= @exp_tolevelup
      @level += 1
      calculate_effort_values(target)
      @stat = {
        hp: ((((2 * @base_stats[:hp]) + @indivudal_stats[:hp] + (@effort_values[:hp] / 4.0).floor) * @level / 100) + level + 10).floor,
        attack: ((((2 * @base_stats[:attack]) + @indivudal_stats[:attack] + (@effort_values[:attack] / 4.0).floor) * level / 100) + 5).floor,
        defense: ((((2 * @base_stats[:defense]) + @indivudal_stats[:defense] + (@effort_values[:defense] / 4.0).floor) * level / 100) + 5).floor,
  ...
  def offensive_stat(type_attack, enemy_type)
    ...
    types_multipler.each do |moves|
      multiplier_damage = moves[:multiplier] if moves[:user] == type_attack && moves[:target] == enemy_type[0]
    end
    multiplier_damage # [0.5]
  end

  def target_defensive_stat(type_attack, enemy)
    type_attack == :normal ? enemy.stat[:defense] : enemy.stat[:special_defense]
  end

  def calculate_effort_values(enemy)
    effort_po = enemy.effort_points
    effort_va = effort_values

    effort_va.each do |key, value|
      value += effort_po[:amount] if key == effort_po[:type]
    end
